...
async function extractStreamUrl(html) {
    ...
    if (!containerMatch) return JSON.stringify({ streams: [] });
    const containerHTML = containerMatch[1];

    const extractors = [
        { regex: /<a[^>]*data-src="([^"]*mp4upload\.com[^"]*)"[^>]*>\s*(?:<span[^>]*>)?([^<]*)<\/span>/gi, label: "Mp4upload", extractor: mp4Extractor },
        { regex: /<a[^>]*data-src="([^"]*uqload\.net[^"]*)"[^>]*>\s*(?:<span[^>]*>)?([^<]*)<\/span>/gi, label: "Uqload", extractor: uqloadExtractor },
        { regex: /<a[^>]*data-src="(\/\/vidmoly\.to[^"]*)"[^>]*>\s*(?:<span[^>]*>)?([^<]*)<\/span>/gi, label: "Vidmoly", extractor: vidmolyExtractor },
        { regex: /<a[^>]*data-src="([^"]*vkvideo\.ru[^"]*)"[^>]*>\s*(?:<span[^>]*>)?([^<]*)<\/span>/gi, label: "VKVideo", extractor: vkvideoExtractor }
    ];

    for (const { regex, label, extractor } of extractors) {
        const matches = [...containerHTML.matchAll(regex)];
