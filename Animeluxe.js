...
function extractEpisodes(html) {
    ...
    if ((matches = html.match(htmlRegex))) {
        matches.forEach(link => {
            ...
            const number = link.match(/الحلقة\s*(\d+)/i)?.[1];
            if (hrefMatch && number) {
                episodes.push({
                    href: "https://ww3.animeluxe.org" + hrefMatch[1],
                    number: number
                });
            }
        });
...
// (Rest of extractStreamUrl, mp4Extractor, uqloadExtractor, etc. omitted for brevity - included in full earlier)
